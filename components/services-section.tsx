...
export function ServicesSection() {
  const cards = services.map((service, index) => (
    <div
      key={index}
      className="group p-8 bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 h-full"
    >
      ...
      <h3 className="text-xl font-bold text-[#0D1826] mb-3">{service.title}</h3>
      <p className="text-[#333C33]/70 text-sm leading-relaxed mb-6">{service.description}</p>
